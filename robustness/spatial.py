import torch as ch
import pdb
from liegroups.torch import SE2, SO2, utils, SO3
...
def make_mats(rots, txs):
    # rots: degrees
    # txs: % of image dim
    rots = rots * 0.01745327778 # deg to rad (64)
    txs = txs * 0.01745327778  #* 2  # (64, 2)
    # 10, 10, 30
    w = ch.cat([txs, rots[...,None]],1).detach().cpu()
    mats = SO3.exp(w).mat.cuda()
...
def make_mats_orig(rots, txs):
    # rots: degrees
    # txs: % of image dim

    rots = rots * 0.01745327778 # deg to rad (64)
    txs = txs * 0.01745327778  # (64, 2)
